...
The system SHALL provide `set_fan_speed(speed)`, `set_mop_mode(mode)`, and `set_water_flow(flow)` methods that persist device-level defaults without starting a clean.
